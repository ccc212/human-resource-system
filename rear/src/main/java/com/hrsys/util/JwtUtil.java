...
public class JwtUtil {
    ...
    private static final Map<String, Date> invalidTokens = new ConcurrentHashMap<>();
    ...
    public static Claims parseJWT(String secretKey, String token) {
        ...
        if (invalidTokens.containsKey(token)) {
            throw new BizException(StatusCodeEnum.INVALID_TOKEN);
        }
    ...
    public static void invalidateToken(String token) {
        ...
        invalidTokens.entrySet().removeIf(entry ->
                (currentTime - entry.getValue().getTime()) > 24 * 60 * 60 * 1000);
    }
}
